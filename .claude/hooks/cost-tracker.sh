...
# ============================================================
# Counts TOOL INVOCATIONS per session (one log line per tool call).
# NOTE: this is an invocation counter, NOT a token or dollar meter. A
# PostToolUse hook cannot see token usage or cost, so it does not measure
# them — any $/token figure here would be fabricated and is reported as n/a.
# Use it to see which tools dominate a session, nothing more.
# Appends structured entries to session-costs.log. Always returns success.
# ============================================================
...
if [ "$((LINE_COUNT % 50))" -eq 0 ] && [ "$LINE_COUNT" -gt 0 ]; then
  ...
  echo "--- Session $SESSION_DATE — top 5 tools by INVOCATION count (not tokens/\$): $TOOL_SUMMARY ---" >> "$LOG_FILE"
fi
