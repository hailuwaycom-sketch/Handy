...
**Honestidad:** los tokens se **estiman** (`chars / 4`) — repórtalos con `~`, no
como cifras exactas. Si un check no tiene datos (log vacío, sin memoria, etc.),
su resultado es **n/a** y se **excluye** del cómputo (repartir sus 12.5 pts entre
los checks que sí corrieron) — nunca contarlo como 🔴 0 pts, que fabricaría un
score peor de lo real. El score es orientativo; lo accionable son los top-5 fixes.
