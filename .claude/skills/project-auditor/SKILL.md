...
## Honestidad de métricas (sin números inventados)

- **Cada hallazgo cita evidencia directa:** `archivo:línea` o la salida del
  comando que lo detectó (ripgrep, `get_advisors`). Sin evidencia no es un
  hallazgo, es una corazonada — no lo reportes.
- **n/a, nunca un número fabricado:** si una dimensión no se pudo medir de
  verdad (p. ej. el MCP de Supabase no respondió → sin advisors), su sub-score
  es **n/a** y se **excluye** del compuesto (re-normalizar los pesos entre las
  dimensiones que sí corrieron). No rellenar con 100 ni con 0.
- **El compuesto es orientativo, no calibrado.** Sirve para ver tendencia entre
  snapshots (`/temple compare`), no como veredicto. Lo que decide es la lista de
  hallazgos citados y la regla de bloqueo de arriba — no el número.
