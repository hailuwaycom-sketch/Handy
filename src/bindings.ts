...
async updateTextReplacementRules(rules: TextReplacementRule[]) : Promise<Result<null, string>> {
    try {
    return { status: "ok", data: await TAURI_INVOKE("update_text_replacement_rules", { rules }) };
} catch (e) {
    if(e instanceof Error) throw e;
    else return { status: "error", error: e  as any };
}
...
export type TextReplacementRule = { id: string; find: string; replace: string; use_regex?: boolean; case_sensitive?: boolean }
/**
